...
if sys.version_info[0] == 3:
    ...
    _strxor = lambda str1, str2: "".join(
        chr(ord(s1) ^ ord(s2)) for (s1, s2) in zip(str1, str2)
    )
...
class XOFExpander(Expander):
    def __init__(self, dst, hash_fn, security_param):
        ...
        super(XOFExpander, self).__init__(
            "expand_message_xof", dst, dst_prime, hash_fn, security_param
        )
...
def test_dst(suite_name, L=0):
    ...
    dst = "-".join(
        filter(None, ["QUUX-V01-CS02-with", suite_name, "1" * max(0, L - length)])
    )
    return dst

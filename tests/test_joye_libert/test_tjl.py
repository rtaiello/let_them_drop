...
from src.buildingblocks import TJLS, VES
...
def test_scheme(threshold, nusers, drops):
    ...
    tjls_instance = TJLS(threshold, nusers, ve)
    users = list(range(1, nusers + 1))
    alive = list(range(1, nusers - drops + 1))
    dropped = list(range(nusers - drops + 1, nusers + 1))

    pp, server_key, users_key = tjls_instance.setup(keysize)

    shares = {}
    for u in users:
        shares[u] = tjls_instance.sk_share(users_key[u - 1])
    ...
    ys = []
    for u in alive:
        y = tjls_instance.protect(pp, users_key[u - 1], tau, xs[u - 1])
        ys.append(y)
    if dropped != []:
        yzero_shares = {}
        for u in alive:
            ...
            yzero_shares[u] = tjls_instance.share_protect(pp, dropped_users_shares, tau)

    if dropped != []:
        yzero_shares = list(yzero_shares.values())[:threshold]
        yzero = tjls_instance.share_combine(pp, yzero_shares, tjls_instance.threshold)
    else:
        yzero = None
    sum_protocol = tjls_instance.agg(pp, server_key, tau, ys, yzero)
